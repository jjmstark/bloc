...
function scaffold(projectName) {
    ...
    if (stat !== undefined) {
        ...
        var projectFolders = ['js', 'html', 'css', 'routes', 'contracts', 'meta', 'views', 'app'];

        var templatesFolder =
            path.normalize(path.join(__dirname, '..', 'templates'));

        readdirp({ root: path.join(templatesFolder), fileFilter: '*.+(js|hbs|template|sol|css)' })
