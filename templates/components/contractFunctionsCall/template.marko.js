function create(__helpers) {
  ...
  return function render(data, out) {
    out.w('<div id="functionsContainer"> ');
      ...
      forEachProp(contract.symTab, function(symbol,symbolObj) {
        ...
        if (symbolObj.jsType == 'Function') {
          out.w('<div><div id="' +
            ...
            'Div"><button class="btn col-sm-12" onclick="callFunc(&#39;' +
            escapeXmlAttr(symbol) +
            '&#39;)">' +
            escapeXml(symbol) +
            '</button> ');
          ...
          forEach(symbolObj.functionArgs, function(argName) {
            out.w('<div><input class="form-control col-sm-12" type="text"' +
              attr("name", argName) +
              attr("placeholder", argName) +
              ' id="' +
              escapeXmlAttr(symbol) +
              escapeXmlAttr(argName) +
              '"> <input class="form-control col-sm-12" type="value" name="' +
              escapeXmlAttr(symbol) +
