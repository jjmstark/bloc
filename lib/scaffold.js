...
function inflateIfTemplate(fileData,filepath) {
    return fileData;
}
...
function scaffold(projectName,developer) {
    ...
    if (stat !== undefined) {
        ...
        readdirp({ root: path.join(templatesFolder), fileFilter: '*.+(js|hbs|scaffold|template|sol|css|json)' })
            ...
            .pipe(through.obj(
				...
				.pipe(through.obj(
					...
					this.push(inflateIfTemplate(fileData,entry.fullPath));
					cb();
			            }))
